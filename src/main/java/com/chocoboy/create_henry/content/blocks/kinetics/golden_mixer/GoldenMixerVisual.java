package com.chocoboy.create_henry.content.blocks.kinetics.golden_mixer;
...
public class GoldenMixerVisual extends SingleAxisRotatingVisual<GoldenMixerBlockEntity> implements SimpleDynamicVisual {
    private final RotatingInstance mixerHead;
    private final OrientedInstance mixerPole;
    private final GoldenMixerBlockEntity mixer;

    public GoldenMixerVisual(VisualizationContext context, GoldenMixerBlockEntity blockEntity, float partialTick) {
        super(context, blockEntity, partialTick, Models.partial(AllPartialModels.SHAFTLESS_LARGE_COGWHEEL));
        this.mixer = blockEntity;

        mixerHead = instancerProvider().instancer(AllInstanceTypes.ROTATING, Models.partial(HenryPartialModels.GOLDEN_MIXER_HEAD)).createInstance();
        mixerHead.setRotationAxis(Direction.Axis.Y);

        mixerPole = instancerProvider().instancer(InstanceTypes.ORIENTED, Models.partial(HenryPartialModels.GOLDEN_MIXER_POLE)).createInstance();
        animate(partialTick);
    }
