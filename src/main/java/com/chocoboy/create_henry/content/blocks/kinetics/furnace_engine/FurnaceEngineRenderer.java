    ...
    protected void renderSafe(FurnaceEngineBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        if (!Backend.canUseInstancing(be.getLevel())) {
            ...
            if (angle != null) {
                ...
                this.transformed(HenryPartialModels.ENGINE_PISTON, blockState, facing, roll90).translate(0.0, piston, 0.0).light(light).renderInto(ms, vb);
                this.transformed(HenryPartialModels.ENGINE_LINKAGE, blockState, facing, roll90).centre().translate(0.0, 1.0, 0.0).unCentre().translate(0.0, piston, 0.0).translate(0.0, 0.25, 0.5).rotateX(sine2 * 23.0F).translate(0.0, -0.25, -0.5).light(light).renderInto(ms, vb);
                this.transformed(HenryPartialModels.ENGINE_CONNECTOR, blockState, facing, roll90).translate(0.0, 2.0, 0.0).centre().rotateXRadians(-angle + 1.5707964F).unCentre().light(light).renderInto(ms, vb);
            }
        }
    }
