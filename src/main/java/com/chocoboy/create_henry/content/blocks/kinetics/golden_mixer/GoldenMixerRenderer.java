...
public class GoldenMixerRenderer extends KineticBlockEntityRenderer<GoldenMixerBlockEntity> {
    public GoldenMixerRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public boolean shouldRenderOffScreen(@NotNull GoldenMixerBlockEntity be) {
        return true;
    }

    @Override
    protected void renderSafe(GoldenMixerBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        ...
        var poleRender = CachedBuffers.partial(HenryPartialModels.GOLDEN_MIXER_POLE, blockState);
        poleRender.translate(0, -renderedHeadOffset, 0).light(light).renderInto(ms, vb);

        var headRender = CachedBuffers.partial(HenryPartialModels.GOLDEN_MIXER_HEAD, blockState);
